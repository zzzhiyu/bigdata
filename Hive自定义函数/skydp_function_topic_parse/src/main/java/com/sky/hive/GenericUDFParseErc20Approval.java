...
@Description(
        name = "parse_erc20_approval",
        value = "if topics are erc20 approval return string (format:owner-spender-value)" +
                "else retun null",
        extended = "参数个数2个，解析erc20的topic和data"
)

public class GenericUDFParseErc20Approval extends UDF {
    public String evaluate(String[] arguments) throws UDFArgumentLengthException {
        ...
        //Check whether approval method is used
    ...
    private String methodSha3Str() {
        byte[] bytes = "Approval(address,address,uint256)".getBytes(StandardCharsets.UTF_8);

...
public class RuleParserUtils {
    ...
    public static int replaceExecuteSqlPlaceholder(List<DqRuleExecuteSql> executeSqlList,
        ...
        for (DqRuleExecuteSql executeSqlDefinition : midExecuteSqlDefinitionList) {
            if (executeSqlDefinition.getTableAlias().equalsIgnoreCase(RuleManager.TIMELINES_ITEMS)) {
                executeSqlDefinition.setSql(RuleManager.TIMELINES_ITEMS_SQL);
            } else if (executeSqlDefinition.getTableAlias().equalsIgnoreCase(RuleManager.DAY_RANGE)) {
                executeSqlDefinition.setSql(RuleManager.DAY_RANGE_SQL);
            } else if (executeSqlDefinition.getTableAlias().equalsIgnoreCase(RuleManager.WEEK_RANGE)) {
                executeSqlDefinition.setSql(RuleManager.WEEK_RANGE_SQL);
            } else if (executeSqlDefinition.getTableAlias().equalsIgnoreCase(RuleManager.MONTH_RANGE)) {
                executeSqlDefinition.setSql(RuleManager.MONTH_RANGE_SQL);
            } else if (executeSqlDefinition.getTableAlias().equalsIgnoreCase(RuleManager.LAST_SEVEN_DAYS)) {
                executeSqlDefinition.setSql(RuleManager.LAST_SEVEN_DAYS_SQL);
            } else if (executeSqlDefinition.getTableAlias().equalsIgnoreCase(RuleManager.LAST_THIRTY_DAYS)) {
                executeSqlDefinition.setSql(RuleManager.LAST_THIRTY_DAYS_SQL);
            }
        }
        // TODO END
        ...
        //TODO 修改的部分  保存前面查询的sql
        String middleTable = null;
        String middleSql = null;
        //TODO 结束
        ...
        if (CollectionUtils.isNotEmpty(midExecuteSqlDefinitionList)) {
            ...
            for (DqRuleExecuteSql executeSqlDefinition:midExecuteSqlDefinitionList) {
                ...
                //TODO 修改的部分 保存之前执行的sql
                if (preIndex < index) {
                    BaseConfig baseConfig = transformerConfigList.get(transformerConfigList.size() - 1);
                    Map<String, Object> config = baseConfig.getConfig();
                    String tableName = config.get(OUTPUT_TABLE).toString().toLowerCase();
                    if (Arrays.asList(RuleManager.OUTPUT_TABLES).contains(tableName)) {
                        // 修改表名
                        config.put(OUTPUT_TABLE, RuleManager.COMPARISON_TABLE);
                    } else {
                        middleTable = tableName;
                        middleSql = " (" + config.get(SQL).toString() + " ) as " + middleTable + " ";
                        // 修改表名
                        config.put(OUTPUT_TABLE, "tmp" + preIndex);
                    }
                }
                //TODO 结束
            }
        ...
        if (CollectionUtils.isNotEmpty(statisticsExecuteSqlDefinitionList)) {
            ...
            for (DqRuleExecuteSql executeSqlDefinition:statisticsExecuteSqlDefinitionList) {
                ...
                if (preIndex < index) {
                    BaseConfig baseConfig = transformerConfigList.get(transformerConfigList.size() - 1);
                    Map<String, Object> config = baseConfig.getConfig();
                    String sql = config.get(SQL).toString().toLowerCase();
                    if (middleTable != null && sql.contains(middleTable)) {
                        config.put(SQL, sql.replace(middleTable, middleSql));
                    }
                    config.put(OUTPUT_TABLE, RuleManager.STATISTICS_TABLE);
                }
                //TODO 修改结束

...
import com.skydp.bean.dws.log.*;
import com.skydp.bean.dws.transaction.OpenseaFee;
import com.skydp.bean.dws.transaction.Side;
...
public class ParseNFTTrades extends TableFunction<String> {
    //获取opensea的税收
    ...
    private transient final HashMap<String, ArrayList<Erc1155Transfer>> erc1155Map = new HashMap<>();
    ...
    public ArrayList<OpenSea> parseStrToOpenseaList(String param) {
        ArrayList<OpenSea> openSeaList = new ArrayList<>();
        ...
        Transaction transaction = new Transaction(
                new Long(txnMsg[0].length() != 0? txnMsg[0]: "0"),
                txnMsg[1], txnMsg[2], txnMsg[3], txnMsg[4],
                new Integer(txnMsg[5].length() != 0? txnMsg[5]: "0")
        );
...
//                System.out.println("---------------------");
    ...
    private OpenSea parseOpenSea(Transaction transaction, Log log, int count) {
        ...
        openSea.setDt(transaction.getBlockTime());
        ...
        //原始代币信息
        openSea.setOriginalAmountRaw(
                new BigInteger(order.getPrice() != null ? order.getPrice(): "0"));
    ...
    private void getEthFee(OpenSea openSea, int count) {
        ...
        if ("ETH".equals(openSea.getOriginalCurrency())) {
            ...
            if (openseaFee.getPlatformFeesPercentForSeller().compareTo(zero) > 0) {
                BigDecimal feeValue = openSea.getOriginalAmount().multiply(openseaFee.getPlatformFeesPercentForSeller());
                ...
                BigDecimal feeValue = openSea.getEthPlatformFeesForSeller().multiply(ToolUtil.strToBigDecimal(String.valueOf(Math.pow(10, -18))));
                openSea.setTokenPlatformFeesForSeller(feeValue);
                openSea.setEthPlatformFeesForSeller(feeValue);
            }
            ...
            if (openseaFee.getPlatformFeesPercentForBuyer().compareTo(zero) > 0) {
                BigDecimal feeValue = openSea.getOriginalAmount().multiply(openseaFee.getPlatformFeesPercentForBuyer());
                ...
                BigDecimal feeValue = openseaFee.getEthPlatformFeesForBuyer().multiply(ToolUtil.strToBigDecimal(String.valueOf(Math.pow(10, -18))));
                openSea.setTokenPlatformFeesForBuyer(feeValue);
                openSea.setEthPlatformFeesForBuyer(feeValue);
            }
        }
    ...
    private void getTokenFee(OpenSea openSea, String key, Side side) {
        ...
        for (Erc20Transfer erc20Transfer : erc20Transfers) {
            ...
            BigDecimal tokenAmount =
                    ToolUtil.strToBigDecimal(erc20Transfer.getValue()).multiply(ToolUtil.strToBigDecimal(String.valueOf(Math.pow(10, -erc20Transfer.getDecimal()))));
            if (side == Side.Sell) {
                BigDecimal value = openSea.getTokenPlatformFeesForSeller().add(tokenAmount);
                ...
                BigDecimal value = openSea.getTokenPlatformFeesForBuyer().add(tokenAmount);
    ...
    private boolean getErc1155NFT(OpenSea openSea, String seller, String buyer) {
        ...
        ArrayList<Erc1155Transfer> erc1155TransferSingles = erc1155Map.get(key);
        if (erc1155TransferSingles != null && erc1155TransferSingles.size() != 0) {
            openSea.setErcStandard("ERC1155");
            for (Erc1155Transfer erc1155TransferSingle : erc1155TransferSingles) {
                //拼接nft_id字符串
    ...
    private void getTokenMsg(OpenSea openSea, String orderSeller, String orderBuyer) {
        ...
        //获取ETH的币价
        BigDecimal ethValue =
                new BigDecimal(openSea.getOriginalAmountRaw()).multiply(ToolUtil.strToBigDecimal(String.valueOf(Math.pow(10, -18))));
        //计算token的值，去精度
    ...
    private boolean getErc20TokenMsg(OpenSea openSea, String key) {
        ...
        for (Erc20Transfer erc20Transfer : erc20Transfers) {
            if (openSea.getOriginalAmountRaw() != null &&
                    openSea.getOriginalAmountRaw().toString().equals(erc20Transfer.getValue())) {
                //计算token的值，去精度
                BigDecimal tokenValue =
                        new BigDecimal(openSea.getOriginalAmountRaw())
                                .multiply(ToolUtil.strToBigDecimal(String.valueOf(Math.pow(10, -erc20Transfer.getDecimal()))));
                //设置币价格
    ...
    private boolean parseErc1155Transfer(Log log) {
        ...
        if ((ERC1155_TRANSFER_SINGLE_METHOD.equals(method) || ERC1155_TRANSFER_BATCH_METHOD.equals(method))
            ...
            Erc1155Transfer erc1155Case = new Erc1155Transfer(
